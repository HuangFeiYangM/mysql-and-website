...
**`git fetch` 和 `git push` 是 Git 中用于与远程仓库（如 GitHub、GitLab 等）交互的核心命令，分别承担“拉取”和“推送”的角色。** 以下是它们的详细解释和实际应用场景：

---

#### **1. `git fetch`：获取远程仓库的最新状态**
#### **作用**
- **从远程仓库下载最新提交、分支和标签信息到本地**，但**不会自动合并**到你的当前工作目录。
- 它更像是“检查远程仓库有什么新内容”，而不是直接修改你的本地代码。

#### **使用场景**
- 查看团队其他成员是否推送了新代码。
- 获取远程仓库新增的分支或标签。
- 在合并或拉取代码前，先查看远程仓库的更新情况。

#### **命令示例**
```bash
git fetch origin   # 从远程仓库 origin 获取所有分支的最新提交
git fetch origin main  # 仅获取 origin 的 main 分支的更新
```

#### **关键点**
- `git fetch` 后，远程分支的更新会被保存到本地的“远程跟踪分支”（如 `origin/main`）。
- 查看远程分支的更新内容：
  ```bash
  git log origin/main  # 查看远程 main 分支的提交历史
  ```
- 合并远程分支的更新到本地分支（需手动操作）：
  ```bash
  git checkout main    # 切换到本地 main 分支
  git merge origin/main  # 将远程 main 分支的更新合并到本地
  ```
...
#### **作用**
- **将本地的提交（Commit）上传到远程仓库**，使其他人可以获取你的代码。
- 如果远程仓库已有新提交，需先拉取（`git pull`）并解决冲突，才能推送。
...
git push -u origin new-feature  # 推送本地 new-feature 分支到远程，并建立跟踪关系（-u）
```

#### **关键点**
- **权限要求**：你必须有远程仓库的写入权限（否则会报错 `Permission denied`）。
- **分支命名**：推送时默认本地分支和远程分支同名。如果远程不存在同名分支，Git 会新建它。
- **强制推送（慎用）**：`git push --force` 会覆盖远程分支的历史，仅用于紧急修正（可能破坏他人代码）。

---

#### **`git fetch` vs `git push` 对比**
| 特性                | `git fetch`                  | `git push`                  |
|--------------------|-----------------------------|-----------------------------|
| **方向**           | 从远程仓库拉取更新到本地         | 从本地推送更新到远程仓库         |
| **是否修改本地代码** | 不影响本地工作目录              | 不影响本地代码，只上传提交       |
| **自动合并**        | 需手动合并（如 `git merge`）   | 无自动合并，可能需先解决冲突     |
| **常用场景**        | 检查远程更新、同步分支信息        | 共享代码、发布新功能或修复       |
...
1. **拉取远程最新代码**（避免冲突）：
   ```bash
   git fetch origin    # 查看远程是否有更新
   git merge origin/main  # 合并远程 main 分支到本地
   ```
2. **本地开发并提交**：
   ```bash
   git checkout -b feature/login  # 新建分支
   # 修改代码...
   git add .
   git commit -m "实现登录功能"
   ```
3. **推送到远程仓库**：
   ```bash
   git push -u origin feature/login  # 首次推送需指定远程分支名
   ```
4. 其他成员可通过 `git fetch` 看到你的分支，并拉取到本地：
   ```bash
   git fetch origin
   git checkout feature/login  # 切换到你的分支
   ```

---

#### **常见问题**
#### **1. 推送时报错 `non-fast-forward`**
- **原因**：远程分支已有本地不包含的新提交。
- **解决**：
  ```bash
  git pull origin main  # 先拉取远程更新并合并
  git push origin main  # 重新推送
  ```

#### **2. 推送时提示权限不足**
- **原因**：未配置 SSH 密钥或 HTTPS 凭据。
- **解决**：
  - **SSH**：将公钥（`~/.ssh/id_rsa.pub`）添加到 GitHub → Settings → SSH Keys。
  - **HTTPS**：使用个人访问令牌（Token）替代密码（GitHub 已禁用密码验证）。
